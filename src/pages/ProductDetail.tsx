...
//                           <p className="text-sm font-mono font-bold text-black">{currentSKU || "N/A"}</p>
//                         </div>
//                       </div>
  ...
  try {
    ...
    return (
      <Layout title={`${product.name || 'Product'} | Retro Roots`} description={product.description || `View details for ${product.name}`}>
        ...
        {/* Page Header Banner */}
        <div className="relative py-6 md:py-8 text-center overflow-hidden">
          <div className="absolute inset-0 bg-black/80 z-0" />
          <div className="relative z-10 container mx-auto px-4">
            <h1 className="text-lg md:text-xl font-bold text-white mb-1">Product Details</h1>
            <p className="text-white/70 text-xs max-w-2xl mx-auto">Discover premium craftsmanship</p>
          </div>
        </div>
        ...
        <section className="py-4 bg-background">
          <div className="container mx-auto px-4 max-w-6xl">
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 mb-10">
              ...
              {/* Left Side: Image Gallery */}
              <div className="w-full lg:w-[40%] flex flex-col gap-3 lg:sticky lg:top-24 h-fit relative z-10">
                <div className="w-full aspect-square bg-white rounded-xl border border-border flex items-center justify-center p-3 shadow-md group relative">
                  <ImageMagnifier
                    src={selectedImage || "/placeholder.svg"}
                    zoomSrc={selectedImage || "/placeholder.svg"}
                    alt={product.name || "Product"}
                  ...
                  {(!isAvailable || (selectedVariant ? selectedVariant.stock === 0 : !inStock)) && (
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] z-20 flex items-center justify-center pointer-events-none rounded-xl">
                      <div className="bg-rose-600 text-white px-6 py-2 rounded-full font-black text-base shadow-2xl transform -rotate-12 border-4 border-white animate-pulse">
                        OUT OF STOCK
                      </div>
                    </div>
                  )}
                    ...
                    {imagesList.map((img: any, idx: number) => {
                      ...
                      return (
                        <button
                          ...
                          onMouseEnter={() => setSelectedImage(url)}
                          onClick={() => setSelectedImage(url)}
                          className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${selectedImage === url ? 'border-primary ring-2 ring-primary/20 shadow-md' : 'border-border opacity-70 hover:opacity-100'}`}
                        >
                          <img src={url} alt={`${product.name} thumbnail ${idx + 1}`} className="w-full h-full object-cover" />
                        </button>
                      );
              ...
              <div className="w-full lg:w-[60%] space-y-5">
                ...
                <div>
                  <Link to="/products" className="text-xs font-bold text-primary hover:underline uppercase tracking-widest mb-2 block">
                    {categoryName}
                  </Link>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex items-center text-amber-400">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < Math.floor(rating) ? 'fill-current' : i < rating ? 'fill-current opacity-50' : 'text-gray-300'}`} />
                      ))}
                    </div>
                    <span className="text-xs font-medium text-muted-foreground">{rating} / 5.0 ({reviewsCount} verified reviews)</span>
                  </div>

                  <div className="flex items-end gap-3 mb-5">
                    <span className="text-2xl font-bold text-foreground">₹{(selectedVariant?.price !== undefined ? selectedVariant.price : currentPrice).toLocaleString()}</span>
                    {discount > 0 && (
                      <>
                        <span className="text-sm text-muted-foreground line-through mb-0.5">₹{originalPrice.toLocaleString()}</span>
                        <Badge className="bg-emerald-100 text-emerald-700 mb-1 border-emerald-200 text-xs">-{discount}% OFF</Badge>
                      </>
                    )}
                  </div>
                ...
                <div className="space-y-5 p-4 bg-muted/30 rounded-2xl border border-border/50">
                  ...
                  {product.seats && Array.isArray(product.seats) && product.seats.length > 0 && (
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-wider text-black mb-3">Seating Capacity</h3>
                      <div className="flex flex-wrap gap-2">
                        {product.seats.map((seat: string) => (
                          <button
                            ...
                            className={`px-4 py-2 text-xs font-black rounded-full border-2 transition-all duration-300 ${selectedSeat === seat
                              ? 'border-primary bg-primary text-black shadow-md'
                              : 'border-border bg-white hover:border-primary/50 text-black'}`}
                          >
                            {seat}
                          </button>
                        ))}
                  ...
                  {product.colors && Array.isArray(product.colors) && product.colors.length > 0 && (
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-wider text-black mb-3">Color Options</h3>
                      <div className="flex flex-wrap gap-3">
                        {product.colors.map((color: string) => {
                          const colorMap: Record<string, string> = {
                            "Beige": "#F5F5DC",
                ...
                ${selectedColor === color
                  ...
                  {product.sizes && Array.isArray(product.sizes) && product.sizes.length > 0 && (
                    <div>
                      <h3 className="text-xs font-bold uppercase tracking-wider text-black mb-3">Select Size</h3>
                      <div className="flex flex-wrap gap-2">
                        {product.sizes.map((size: string) => (
                          <button
                            key={size}
                            ...
                            className={`px-4 py-2 text-xs font-bold rounded-full border-2 transition-all duration-300 ${selectedSize === size
                              ? 'border-primary bg-primary text-black shadow-md'
                              : 'border-border bg-white hover:border-primary/50 text-black'}`}
                          >
                            {size}
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-4 pt-3 border-t border-border/50">
                    <div className="flex flex-col gap-1.5 w-full md:w-auto">
                      ...
                      <div className="flex items-center gap-1 p-1 bg-white rounded-full border-2 border-border w-fit shadow-sm">
                        <button
                          onClick={() => setQuantity(Math.max(1, quantity - 1))}
                          disabled={quantity <= 1}
                          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-muted transition-colors disabled:opacity-30 font-bold"
                        >
                        ...
                        <button
                          onClick={() => setQuantity(Math.min(selectedVariant?.stock || 99, quantity + 1))}
                          disabled={selectedVariant ? quantity >= selectedVariant.stock : false}
                          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-muted transition-colors font-bold"
                        >
                    ...
                    <div className="flex-1 w-full space-y-3">
                      <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border/50">
                        <div className="flex flex-col">
                          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-0.5">Availability</span>
                          {getStockStatusLabel()}
                        </div>
                        <div className="text-right">
                          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-0.5">SKU</span>
                          <p className="text-xs font-mono font-bold text-black">{currentSKU || "N/A"}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button
                          disabled={!isAvailable || (selectedVariant ? selectedVariant.stock === 0 : !inStock)}
                          className="h-11 rounded-full text-sm font-bold shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all disabled:bg-muted disabled:text-muted-foreground"
                        >
                          {(!isAvailable || (selectedVariant ? selectedVariant.stock === 0 : !inStock)) ? (
                        ...
                        <Button
                          variant="outline"
                          disabled={!isAvailable || (selectedVariant ? selectedVariant.stock === 0 : !inStock)}
                          className="h-11 rounded-full text-sm font-bold border-2 hover:bg-muted transition-all disabled:opacity-50"
                        >
                          {(!isAvailable || (selectedVariant ? selectedVariant.stock === 0 : !inStock)) ? "Notify Me" : "Buy Now"}
                        </Button>
                      </div>
            ...
            <div className="max-w-4xl">
              <div className="mb-10">
                <h2 className="text-xl font-bold text-[#C60] mb-3">Product Description</h2>
                <div className="prose prose-sm max-w-none text-foreground/80 leading-loose">
                  {product.description ? (
                    <p className="whitespace-pre-wrap text-sm">{product.description}</p>
                  ) : (
                    <p className="italic text-muted-foreground text-sm">Detailed description is not available for this item at the moment.</p>
                  )}
                </div>
              </div>

              <div>
                <h2 className="text-xl font-bold text-[#C60] mb-3">Product Specifications</h2>
                <div className="border border-border rounded-lg overflow-hidden max-w-2xl">
                  <table className="w-full text-sm text-left">
                    <tbody className="divide-y divide-border">
                      {Object.entries(specifications).map(([key, value], idx) => (
                        <tr key={idx} className={idx % 2 === 0 ? "bg-muted/30" : "bg-white"}>
                          <th className="px-4 py-2.5 font-semibold text-foreground/90 w-1/3 bg-muted/50 border-r border-border text-xs">{key}</th>
                          <td className="px-4 py-2.5 text-foreground/80 text-xs">{typeof value === 'object' ? JSON.stringify(value) : String(value)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
